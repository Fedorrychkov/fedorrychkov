...
Если у вас нет возможности привязать свою карту к firebase, есть несколько путей решения этой проблемы:
  - Завести карту в кокнертной стране самостоятельно
  - Найти сервисы по генерации платежных карт
  - Заказать карту онлайн

Работа с Saas решениями на подобии Firebase не сильно отличаются друг от друга, многие из них не потребуют платежной карты на самом старте. Firebase так же не потребует карту сразу, однако с учетом полноценной конфигурации, карта для работы с Firebase все таки понадобится, как минимум на шаге конфигурации firebase и деплоя индексов. Этот шаг можно пропустить и попытаться поднять бесплатный проект, однако firebase-tools будет выбрасывать ошибки при попытке инцииализации файлов конфигов и управления индексами из локального проекта.

*Примечание*: предложенный стек хорошо подойдет для пет проектов и небольших production проектов. Итоговая бойлерплейт репа будет включать в себя необходимый минимум конфигурации для старта разработки. Если вы ранее не работали с Nodejs и Nest.js в частности, у вас могут возникнуть трудности с некоторыми концепциями и конструкциями кода, заранее рекомендую держать, например [эту статью](https://habr.com/ru/companies/timeweb/articles/663234/)

## Личный опыт
На самом деле, хоть я и начал заниматься fullstack лишь несколько лет назад, мне уже доводилось пробовать в себя в разных проектов, на разных стеках и платформах, в рамках js/ts само собой. С Nestjs я познакомился попав в компанию, которая выбрала путь Fullstack js разработки, с тех пор этот путь и не отпускает. Так вот, за несколько лет работы над разными проектами, мне удалось собрать простой минимум боейлерплейта для очередного MVP стартапа или пет проекта. Я очень надеюсь, что эта статья и итоговая репа, упростит жизнь начинающим бэкендерам.

## Перед инициализацией
Я буду писать этот бойлерплейт со своей точки зрения и окружения, поэтому первым делом: я работаю на платформе MacOS, все предлагаемые действией должны быть кроссплатформенными, но не исключаю трудностей. Буду рад вопросам и рекомендациям в комментариях под этим постом.

Я использую ZSH оболочку, вместо обычного BASH, поэтому первый линк - [ZSH](https://github.com/ohmyzsh/ohmyzsh/wiki/Installing-ZSH).

Так же, у меня есть проблема разных версий nodejs, связано это с тем, что за годы работы накапливаются проекты разной свежести + привычки никто не отменял. Для удобной работы, предлагаю установить Вам [NVM](https://github.com/nvm-sh/nvm).

И напоследок, так как проектов у разработчика может быть много, буду использовать [pnpm](https://pnpm.io/installation)

## Непосредственно инициализация

Перейдем к основному набору команд и конфигурации проекта.

Для начала, давайте установим nodejs 20 версии при помощи nvm.

```nvm i 20 && nvm use 20```

После выполнения этой команды, поставим pnpm. ```npm i -g pnpm```

Предварительно, у вас поставлены такие или чуть моложе версии.
...
Давайте установим nest cli глобально: ```npm i -g @nestjs/cli```, после успешной установки cli можем перейти к шагу с созданием папки проекта. Делается это при помощи команды ```nest new nestjs-startup-boilerplate```, где после слова new вы можете написать название своего проекта. Далее будет предоставлен выбор конфигурации инициализации проекта. 
...
На данный момент, у вас должно получится что-то, вроде [этого](https://github.com/Fedorrychkov/nestjs-startup-boilerplate/commit/33355292acb7111adca3afbd769fd2fc81694efd) набора файлов проекта.

## Конфигурация
Линтинг, tsconfig, env файлы. Давайте законфигурим это все, в конце этой части, я оставлю ссылку на коммиты с в файлах.

В любом проекте будет полезно иметь path aliases, чтобы не часто видеть импорты кода аля ```import something from '../../../../modules/something'```. Мне больше нравится что-то такое: ```import something from '~/modules/something'```. Читать и поддерживать это намного приятнее. В tsconfig файл внесем несколько изменений, объяснять подробно я не буду, если хотите узнать это глубже, придется погуглить.

![tsconfig changes](./images/tsconfig.png?raw=true "tsconfig changes")

Следом, я хочу внести изменения в eslint.js файл, чтобы мне было удобнее работать с проектом на привычном мне конфиге.

![eslint changes](./images/eslintjs.png?raw=true "eslint changes")
Следом, давайте обновим package.json, в scripts добавим следующие команды:
```
"lint": "eslint \"{src,apps,libs,test}/**/*.ts\"",
"lint:fix": "eslint \"{src,apps,libs,test}/**/*.ts\" --fix"
```
И установим новую зависимость:
```
...
ENV файлы. Для начала, добавим наш .env.example. В этом файле, мы примерно подскажем разработчикам, какие переменные можно конфигурить в проекте.
...
По итогу у вас будет вот такой [набор изменений на данном этапе](https://github.com/Fedorrychkov/nestjs-startup-boilerplate/commit/0e3f830d2b55137a9e72e33b32673210cf273642).

А если, вы еще и запустите команду ```pnpm run lint:fix```, то получите исправленные [по линтеру файлы](https://github.com/Fedorrychkov/nestjs-startup-boilerplate/commit/95a73d71132db09df39cef8a8734339893235cde)

И так. На данном этапе, базовый конфиг проекта готов, далее нас ждет: Настройка firebase проекта и непосредственно переменных окружения.

## Подготовка к работе с Firebase

Обычно в своих проекта я использую Firebase, как провайдер: Google Cloud Storage, Firestore Database, Firebase Auth. Давайте настроим все это.
...
Жмакайте на Get Started, приступим к созданию проекта. На 1 экране создания у вас попросят ввести название, я назову свой так: nestjs-boilerplate-example. Это название будет участвовать в будущих настройках env файлов проекта. Далее на 2 экране мне предлагают включить аналитику, мне она не нужна, поэтмоу я отказываюсь и создаю проект. После успешного создания у вас появится проект на дашборде + вы можете перейти к нему нажав на Continue в окне ожидания создания проекта.

В консоле проекта вы увидите что-то наподобии этого.

![Firebase Project Console](./images/firebase_project_console.png?raw=true "Firebase Project Console")

В целом, пока все просто. Далее, нам нужно включить в проекте: Firestore, Authentication, Storage. Эти разделы можно увидеть слева, в сайдбаре консоли, они хранятся в разделе Build.

![Firebase Project Build](./images/firebase_build_sidebar.png?raw=true "Firebase Project Build")

При попадании в каждый раздел, первым делом вы увидите предложение о включении функциональности. Get Started, Create Database и так далее. Просто жмакайте на каждом, до определенных лимитов использования, все это является бесплатным. Дальше, по тарифам. [Подробности по тарифам](https://firebase.google.com/pricing)

Создание базы данных потребует выбора сервера хранилища, в целом, любой удобный Вам, я выбираю обычно Европу (Eur3). + Мод запуска, можете оставить спокойно production режим для БД.

Ну вот, теперь у вас полностью готовый firebase проект. К слову, таким же образом вы можете спокойно генерировать N firebase проектов под необходимые контуры (Production/Stage/Local/Test, в целом мне для жизни очень даже хватает).

Давайте перейдем к следующему этапу. Нам нужно получить необходимые данные для нашего ENV Окружения. Для этого перейдите в ваш [Project Settings](https://console.firebase.google.com/u/2/project/nestjs-boilerplate-example/settings/general) (находится в поповере по клику на шестеренку).

Попав в настройки проекта, выберите вкладку, Service Accounts. И нажмите на кнопку, generate new private key. После этого у вас скачается файл .json формата. Он нам понадобится для шага подключения к firebase.

## Конфигурация проекта для подключения к Firebase.

И так, давайте разберемся с переменными окружения.
Во-первых, скачанный ранее service json файл из firebase, перенесите в корень проекта, и, например, назовите его service-account.json.
Название сервис файла занесите в ваш .env.dev файл, в поле ```SA_KEY=service-account.json```. Env.dev файл можно сделать на основе примера в ```.env.example```.

Так же, нам нужно обновить команды в package.json, раздел scripts.
```
...
```
По сути, мы лишь добавили NODE_ENV переменную. Далее, это значение можно будет достать из process.env.NODE_ENV. Остальные переменные окружений лучше пробрасывать через конкретные .env файлы. Так же для примера, давайте добавим сразу же .env файл (он у нас будет для production режима)

И так, давайте напишем подключение наших env в проекте.
Нам понадобится 2 файла. app.module.ts и main.ts.

Давайте добавим src/env.ts файл и заполним его:
...
```
Таким образом env проекта можно будет использовать по всему проекту.

И еще давайте добавим шаг авторизации и подключения к firebase. Для этого нам понадобится установить пакет ```pnpm add firebase-admin```. Открываем main.ts файл и добавляем инициализацию firebase-admin.
```
...
Итак, теперь у вас есть [стабильный коннект к firebase](https://github.com/Fedorrychkov/nestjs-startup-boilerplate/commit/ba9bf254910bd62c3b8e73c795d1830677e4971b) проекту.

Однако это еще не все! Нам осталось: добавить firebase модуль и пример модели данных с контроллерами и сервисами, а так же gcloud bucket сервис, для хранения файлов.
...
В папке firestore добавляем 4 файла: firestore.module.ts, firestore.providers.ts, types.ts, index.ts чисто для красивого реэкспорта firestore модуля.

```
// firestore.providers.ts
// Этот файл нужен для инициализации вспомогательных классов, для использования в дальнейшем
export const FirestoreDatabaseProvider = 'firestoredb'
export const FirestoreOptionsProvider = 'firestoreOptions'
export const FirestoreCollectionProviders: string[] = [/* Далее нужно будет необходимо добавлять классы документов коллекций firestore */]
```
...
## Example Module
Наконец то мы дошли до создания примера модуля. Мы реализуем базовый CRUD на примере абстрактной сущности example. Конкретно метод delete реализовывать я не стану, смысл его совершенно такой же, разве что вызывать в репозитории надо .delete(documentId) и все, хотя вместо прямого удаления можно реализовать softDelete, который под капотом будет менять поле status у документа (например status: ACTIVE/ARCHIVED). В общем начнем мы с того, что создадим src/modules, в котором и заведем example модуль. Эта сущность нужна лишь для примера того, как можно организовать код проекта. Само собой, Вы вольны делать что угодно и как угодно.

Нам потребуется, например, такая структура модуля.
...
Для дат, в firestore есть подходящая модель Timestamp, с ней достаточно легко работать и фильтровать по ней документы. В качестве id документа будем использовать UUIDv4 утилиту.
...
```
// dto/example.filter.ts
export class ExampleFilter {
  public isPublished?: boolean
}

// dto/example.request.ts
export class ExampleRequestBody {
  public title: string
  public text?: string
}

// dto/example.response.ts
...
Добавим наш контроллер с такими методами, получение списка с query параметрами для фильтрации examples по флагу isPublished, получение деталки example, создание example, редактирование и обновление флага isPublished.
...
Пока что, покрыт тайной лишь файл example.repository, давайте добавим его реализацию, ведь именно он отвечает за походы в firestore.
...
export class ExampleRepository {
  ...
  /**
  * Метод значение документа и доп методы по работе с документом, нас интересует метод .update(...props) 
  **/
...
Так же нам потребуется обновить файл firebase.providers.ts
```
...
И, чтобы данный модуль заработал полноценно, добавим его в app.module.ts файл.
```
...
...
О, кажется мы забыли еще кое-что. Работа с firebase без конфиг файлов firebase не совсем удобна, особенно если речь идет о различных контурах. Нужно будет добавить глобально firebase cli ```npm install -g firebase-tools```. После этого, скорее всего вам потребуется авторизоваться командой ```firebase login```. Как только вы пройдете все требуемые шаги, приступим к конфиг файлам.
В консоле, в корне вашего проекта потребуется ввести команду ```firebase init```, выберите, в моем случае, только Firestore из предлагаемых пунктов. Далее, я если вы как и я, уже создали firebse проект, выберите пункт ```Use an existing project```, найдите в предложенном списке ваш проект и нажмите продолжайте настройку, обычно далее ничего лишнего не нужно менять, дефолтные названия файлов я оставляю как есть.

*Примечание: Выбор осуществляется нажатием на пробел, а продолжение на enter/return (в зависимости от вашей клавиатуры)*
...
Мой лог ошибки инициализации подключения к проекту
...
После создания платежного аккаунта в GCP и привязки проекта к этому аккаунту, firebase-tools пройдет конфигурацию без проблем.

![Firebase init selector](./images/firebase_init.png?raw=true "Firebase init selector")

После того, как вы пройдете все шаги, в проект добавятся такие файлы: .firebaserc, firebase.json, firestore.indexes.json, firestore.rules. Вместо файла .firebaserc в гите мы оставляем его пример, .firebaserc.example. То, как организовать CI/CD для всего этого мы разберем в следующей статье про деплой бэкенда на VPS.
...
В терминале запускаем команду ```pnpm add -D husky```, далее запускаем инициализацию командой ```npx husky init```, это добавит в проект .husky папку, + pre-commit файл, который будет запускаться на стадии коммита изменений. Так же, давайте немного мутируем package.json новой командой в scripts
```
...
"scripts": {
  ...,
...
Этот скрипт нужен для избежания ошибки установки husky, после команды устанвоки зависимостей ```npm i/pnpm i```
...
Теперь, все будущие коммиты будут валидировать ваши js(x)/ts(x) файлы на основе eslint/prettier и добавлять правки для будущего коммита ```lint:fix```. Можно расценивать данный пример прекоммит хука, как основу для ваших личных конфигураций.
Полный набор доступных хуков можно посмотреть в [githooks](https://git-scm.com/docs/githooks)
...
Список example документов без фильтра.

![Postman Example List Get Route](./images/postman_example_list.png?raw=true "Postman Example List Get Route")

В результате мы получим ожидаемую ошибку 404, об отсутствующих документах. (Это поведение изначально заложено в коде контроллере, конфигурировать можно как вам вздумается)

А теперь, давайте запросим список example с фильтром в query параметрах ?isPublished=true

![Postman Example List Get Route With Wrong Filter](./images/postman_example_list.png?raw=true "Postman Example List Get Route With Wrong Filter")

Так, мы послали запрос с параметром, однако получаем ту же ошибку. На самом деле это нормально, но есть проблема. На данный момент, boolean значения наше апи не умеет читать и работает с ними, как со строкой.
...
Давайте попробуем пойти по 2 шагу.

Отредактируем метод контроллера.
...
Таким образом, трансформируем входящий параметр isPublished в boolean параметр.

Давайте создадим новый example документ.

![Postman Example Create](./images/postman_example_create.png?raw=true "Postman Example Create")

Я создал несколько документов, для примера. Давайте еще раз запросим наш списке, но отфилтруем по isPublished=false


![Postman Example Unpublished list](./images/postman_unpublished_example_list.png?raw=true "Postman Example Unpublished list")

Если присмотреться, можно увидеть, что список подтягивается корректно. Однако для многих коллекций требуется менять направления или гарантировать возвращение списка с учетом даты создания по descending/ascending значениям.
...
Поле ```__name__``` не нужно указывать в этом конфиге, при деплое, он добавляется автоматически. Важно соблюдать очередность полей (fields).
...
Итак, теперь можно дополнительно проверить метод получения example документа по id, скрин вставлять не буду, так как этот запрос уже должен работать без проблем. В моем случае, это ```http://localhost:8080/v1/example/7c8a5d30-beca-409a-8509-873616c80f5a```, ваш ID может отличаться от моего.
...
Так же прикрепляю ссылку на текущий набор api вызовов в Postman [json файлик](./common/postman_example.json). Можете скачать его и импортировать в свой воркспейс в Postman для быстрой развертки окружения готовых запросов.

И [текущий коммит](https://github.com/Fedorrychkov/nestjs-startup-boilerplate/commit/81e8040835da547f5f6a2367f51f5104f1ee64f4) по изменениям работы с индексами.
...
- Бесплатный Storage включает предоставляет 5GB хранилище.
- Бесплатный Spark не дает возможности создавать отдельные бакеты, но такой кейс мы учтем в коде. Однако рекомендую использовать дефолтный бакет проекта, а сами файлы на основе коде резолвить по папкам и подпапкам.

Кейсы использования Storage
- Чтение и запись файла во временную папку в корне проекта
...
Давайте приступим, нас ждет еще N файлов.

В папке privders, рядом с firebase, добавим новую папку - bucket.
В ней мы заведем, само собой файл bucket.module.ts и кучу вспомогательных. Чуть не забыл, установим пакет ```pnpm add @google-cloud/storage multer lodash```, и обязательно ```pnpm add -D @types/multer```.
...
Давайте опишем каждый файл, кода будет относительно немного, после добавим добавление + удаление в отдельный ednpoint работы с изображением в api/example/:id/image.
...
Чтобы bucket модуль успешно запустился, нужно импортировать его в app.module.ts проекта
...
Так же нам понадобится добавить helper для валидации файлов
...
Можно заметить, что в модель добавилось поле imageUrl с установкой ссылки из GCloud Storage. Полный набор изменений можно найти в [этом коммите](https://github.com/Fedorrychkov/nestjs-startup-boilerplate/commit/e0e0f789b9459a55729dde26a7a06f3dbe1cd513).

# Заключение
Думаю, что в данной статьей мы достигли цели, описать минимальный конфиг проекта с примером структуры кода, работы с Firestore и GCloud Bucket. Полный пример nestjs проекта можно найти у меня в [GitHub](https://github.com/Fedorrychkov/nestjs-startup-boilerplate). Надеюсь что я смог доступно и с достаточным количеством кода описать основные шаги по генерации CRUD приложения. Я бы не хотел останавливаться на этой статье. Буду рад получить обратную связь и критику. Итоговая репа, может послужить не самым плохим примером для Вашего быстрого старта разработки MVP или пет проекта.

Надеюсь раскрыть в следующих статьях:
- Переносим методы работы с авторизацией и аутентификацией в Firebase в Nest.js API (На базе этой статьи и текущего example проекта)
- Немного разберем деплой этого backend приложения и настроим нотификации в командный чат Telegram
- Попробуем написать Telegram Bot на основе nestjs-startup-boilerplate
- Создадим Mini-App в связке с нашим Telegram ботом.
